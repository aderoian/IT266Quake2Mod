...
void Cmd_SpawnItem_f(edict_t* ent) {
	...
	if (gi.argc() < 2) {
		gi.cprintf(ent, PRINT_HIGH, "Usage: spawnitem <item>\n");
		return;
	}

	//Combine the rest of args
	if (gi.argc() > 2) {
		s = gi.argv(1);
		for (i = 2; i < gi.argc(); i++) {
			strcat(s, " ");
	...
	Com_Printf("ent origin is: (%f, %f, %f)\n", ent->s.origin[0], ent->s.origin[1], ent->s.origin[2]);

	//AngleVectors(ent->client->v_angle, forward, NULL, NULL);
	//Com_Printf("forward is: (%d, %d, %d)\n", forward[0], forward[1], forward[2]);
	//VectorScale(forward, 16, forward);
	//VectorAdd(ent->s.origin, forward, point);
	////VectorScale(forward, 16, point);
	VectorSet(offset, 64, 64, 64);
	VectorAdd(ent->s.origin, offset, point);
	Com_Printf("item origin is: (%f, %f, %f)\n", point[0], point[1], point[2]);
