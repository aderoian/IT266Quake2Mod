...
=================
*/
void stun_touch(edict_t* self, edict_t* other, cplane_t* plane, csurface_t* surf)
{
	int		mod;
	...
	if (other->takedamage)
	{
		other->monsterinfo.stun_end = level.time + 2.0f;
	}
	else
	{
		gi.WriteByte(svc_temp_entity);
...
void fire_stun(edict_t* self, vec3_t start, vec3_t dir, int damage, int speed, int effect, qboolean hyper)
{
	edict_t* bolt;
	trace_t	tr;
